...
- `run_qwen3_4b.py`：fsdp match 自动附加 `--ci-test --ci-disable-kl-checker --ci-disable-logprobs-checker`——`parse_args`（`miles/utils/arguments.py`）把 experimental FSDP backend 挡在 `--ci-test` 后面，不带会直接 `ValueError`（`run_simple.py` 同样带 ci-test）。但 `--ci-test` + `--true-on-policy-mode` 会在 `log_rollout_data` 里断言两侧 logprob 均值**完全相等**，失配直接 AssertionError 杀掉运行——match 要的是离线测量失配，故加 `--ci-disable-logprobs-checker`（`log_utils.py` 的 guard 已扩展为同时覆盖该严格断言）。match 模式 `--skip-train-step` 下 ci-test 的 grad-norm 检查不会执行。
- `fsdp_utils/actor.py`：`--skip-train-step` 生效（原来只有 megatron actor 支持），跳过 optimizer 循环但仍写 dump_details train data。
- `run_qwen3_4b.py`：fsdp match 不设 `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`——expandable segments 的 CUDA 内存走 pidfd 文件描述符共享，SGLang scheduler 反序列化权重时 `pidfd_getfd` 被受限容器拒绝（`RuntimeError: pidfd_getfd: Operation not permitted`，colocate weight sync 即崩）；`run_simple.py` 也不设。非 match 的 fsdp 运行保留该优化。
- `fsdp_utils/actor.py`：true-on-policy 下的 qwen3_moe patch import 改为可容错——`kernels/fused_experts.py` 依赖的 `sglang.srt.layers.moe.fused_moe_triton.fused_moe` 模块路径随 SGLang 版本移动，当前版本缺失时打 warning 跳过（该 patch 只替换 `Qwen3MoeSparseMoeBlock`，dense 模型不需要）；`enable_batch_invariant_mode(enable_bmm=False)` 不受影响照常执行。
- `fsdp_utils/update_weight_utils.py`：weight sync 前后调用 `begin_weight_update` / `end_weight_update`——当前 SGLang 的 `update_weights_from_tensor` 要求先开 weight-update 会话（`AssertionError: requires an open begin_weight_update session`），megatron 路径早已这样做，fsdp（experimental，无人维护）缺失。
